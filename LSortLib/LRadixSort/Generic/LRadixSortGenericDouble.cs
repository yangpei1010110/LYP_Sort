...
{
    public class LRadixSortGenericDouble<TValue> : LRadixSort<TValue>
    {
        private TValue[]             _buffer;
        private Func<TValue, double> _keySelector;

        public LRadixSortGenericDouble(Func<TValue, double> keySelector, int initBufferSize = 0)
        {
        ...
        private void RadixSortSequential(byte byteIndex, TValue[] source, TValue[] dest, int length)
        {
            ...
            {
                long l = BitConverter.DoubleToInt64Bits(_keySelector(source[i]));
